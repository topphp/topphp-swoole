...
use Topphp\TopphpSwoole\server\jsonrpc\exceptions\Exception;
use Topphp\TopphpSwoole\server\jsonrpc\exceptions\ImplementationException;
...
class Server
{
    ...
    private function getInput($json, &$input): bool
    {
        if (!is_string($json)) {
            return false;
        }
     ...
     * Returns null when no response is necessary.
     */
    private function processInput($input): ?array
    {
        if (count($input) === 0) {
     ...
     * Returns null when no response is necessary.
     */
    private function processBatchRequests($input): ?array
    {
        $replies = [];
     ...
     * Returns a response object or an error object.
     */
    private function processQuery($id, $method, $arguments): array
    {
        try {
     ...
     * Array of arguments that will be passed to the method.
     */
    private function processNotification($method, $arguments): void
    {
        try {
     ...
     * Returns an error object.
     */
    private function requestError($id = null): array
    {
     ...
     * Returns an error object.
     */
    private function error($id, $code, $message, $data = null): array
    {
        $error = [
            'code'    => $code,
            'message' => $message
